...
function VerifyEmail() {
  ...
  const handleVerifyAccount = async () => {
    ...
    const response = await fetch(`${process.env.REACT_APP_LOGI_TRACK_BASE_URL}/auth/regenerate-verification-token?email=${email}`, {
      method: 'POST',
      headers: {
    ...
    if (responseData.message === "Token resent successfully, Please check your email to verify your account") {
          ...
          <div className='logitracks'>
            ...
            <div className='form1'>
              ...
              <form>
                ...
                <button type='button' className='submitToVerify' value={email} onClick={() => {
                  { handleVerifyAccount() }
                  setTimeout(() => {
                    setEmail("");
                  }, 500);
              ...
              <p className='verifyAccount'>Already verified your account? <span className='registeraccout' onClick={returnToLogin}>Login</span></p>
            </div>
          </div>
